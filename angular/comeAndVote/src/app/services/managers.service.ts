...
export class ManagersService {
  ...
  checkIsBallotBoxManager(userName:string,password:string):Observable<Managers>{
    return this.http.get<Managers>(`http://localhost:60289/api/Managers/findBallotBoxManager/${userName}/${password}`);
  }

  //שמירת הערכים שהמנהל קלפי הכניס 
  //מספר קלפי ועיר קלפי
  saveNumAndCityBallotBox(numOfBalloBox:string,cityOfBallotBox:string)
  {
